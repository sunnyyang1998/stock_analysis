...
for X, y in all_data:
    ...
    predictions.append((y_pred.flatten(), y))

    # 计算召回率和F1分数（假设y和y_pred都是分类标签）
    recall = recall_score(y, y_pred.round())  # 使用.round()将预测值四舍五入到最接近的整数
    f1 = f1_score(y, y_pred.round())
    total_recall += recall
    total_f1 += f1

final_mse /= total_files
average_recall = total_recall / total_files
average_f1 = total_f1 / total_files
